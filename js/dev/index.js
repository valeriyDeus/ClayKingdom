...
class DynamicAdapt {
  ...
  init() {
    ...
    this.mediaQueries.forEach((media) => {
      ...
      matchMedia2.addEventListener("change", () => this.mediaHandler(matchMedia2, objectsFilter));
...
const initSliders = () => {
  if (document.querySelector(".our-blog__items.swiper")) {
    new Swiper(".our-blog__items.swiper", {
      modules: [Pagination],
      slidesPerView: 1,
      spaceBetween: 30,
      observer: true,
      ...
      on: {
        // Слайдер запущений
        init: (swiper) => {
          const slider = document.querySelector(".our-blog__items.swiper");
          if (slider) {
            const slides = slider.querySelectorAll(".swiper-slide");
            if (slides.length > 0) {
              for (const slide2 of slides) {
                const slideImageHeight = slide2.querySelector(".item-blog__image").offsetHeight;
                slide2.style.setProperty("--slide-image-height", `-${slideImageHeight + 20}px`);
              }
            }
          }
        }
...
const gotoBlock = (targetSelector, config3 = {}) => {
  ...
  const { noHeader, offsetTop } = { ...defaultConfig, ...config3 };
  const getHeaderHeight = () => {
...
const menuInit = () => {
  const burgerIcon = document.querySelector("[data-menu]");
  if (burgerIcon) {
...
class FormsValidation {
  ...
  async formSubmitAction(form, e) {
    ...
    const ajax = form.dataset.formValidation === "ajax";
    const dev = form.dataset.formValidation === "dev";
    try {
      if (ajax) {
  ...
  validateField(formRequiredItem) {
    ...
    if (tagName !== "SELECT" && type !== "checkbox" && type !== "radio" && formRequired === "") {
      if (!formRequiredItem.value.trim()) {
        this.removeFieldSuccess(formRequiredItem);
        this.addFieldError(formRequiredItem, this.config.errorMessages.valueMissing);
        error++;
...
}, random$1 = function random2(min, max, roundingIncrement, returnFunction) {
...
const blockHeaderEffect = () => {
  const blockHeaders = document.querySelectorAll("[data-gsap].block-header");
  blockHeaders.forEach((blockHeader) => {
    const label = blockHeader.querySelector(".block-header__label");
    const title = blockHeader.querySelector(".block-header__title");
    const text = blockHeader.querySelector(".block-header__text");
    const socials = blockHeader.querySelector(".block-header__socials");
    const list = blockHeader.querySelector(".block-header__list");
    const link = blockHeader.querySelector(".block-header__link");
    ...
    if (socials) {
      const socialsItems = socials.querySelectorAll(".socials__item");
      socialsItems.forEach((item, index) => {
        tl.from(item, { y: 200 }, index === 0 ? 0 : "<0.2");
      });
    }
    if (text) {
    ...
    if (list) {
      const listItems = list.querySelectorAll(".block-header__item");
      listItems.forEach((item, index) => {
        tl.from(item, { x: -100 }, index === 0 ? "-=1.5" : "<0.3");
      });
    }
    if (link) {
      if (link.closest(".block-header--blog")) {
        tl.from(link, { x: 100 }, "-=1.5");
        return;
      }
      tl.from(link, { y: 100 }, "-=1.5");
    }
  });
};
const dropDecorEffect = () => {
  const dropDecorItems = document.querySelectorAll("[data-gsap].drop-decor");
  if (!dropDecorItems.length) return;
  dropDecorItems.forEach((dropDecorItem) => {
    initDropDecorAnimation(dropDecorItem);
  });
  function initDropDecorAnimation(dropDecor) {
    const triggerSection = dropDecor.closest("section[data-gsap]");
    const dropElements = dropDecor.querySelectorAll(".drop-decor span");
    const tl = gsapWithCSS.timeline({
      yoyo: false,
      repeat: 0,
      defaults: {
...
const aboutEffect = () => {
  const about = document.querySelector("[data-gsap].about-us");
  if (!about) return;
  const aboutWrapperImages = about.querySelector(".about-us__images-wrapper");
  const aboutDecorImages = aboutWrapperImages.querySelectorAll(".images-decor__item");
  const aboutSmallImage = aboutWrapperImages.querySelector(".about-us__image--small");
  const listProductItems = about.querySelectorAll(".list-production__item");
  const tlImages = gsapWithCSS.timeline({
    defaults: {
      ease: "power4.out",
      duration: 1.6,
      opacity: 0
  ...
  tlImages.from(aboutSmallImage, { y: -200, scale: 1, duration: 2 }, "+=1");
  aboutDecorImages.forEach((el, i) => {
    tlImages.from(
      el,
      {
  ...
  const tlList = gsapWithCSS.timeline({
    defaults: {
      ease: "power4.out",
      duration: 1.2,
      opacity: 0,
    ...
    scrollTrigger: {
      trigger: listProductItems[0],
      start: "top 85%",
      bottom: "bottom 15%"
    }
  });
  listProductItems.forEach((item, i) => {
    tlList.from(item, {}, i === 0 ? 0 : "<0.2");
  });
};
const collectionEffect = () => {
  const collection = document.querySelector("[data-gsap].collection");
  if (!collection) return;
  const collectionItems = collection.querySelectorAll(".item-collection");
  const collectionDecor = collection.querySelector("[data-gsap].decors-collection");
  const tlCollection = gsapWithCSS.timeline({
    defaults: {
      ease: "power4.out",
      duration: 1.2,
      opacity: 0,
  ...
  collectionItems.forEach((item, i) => {
    tlCollection.from(item, {}, i === 0 ? 0 : "<0.3");
  });
  const coolectionDecorInit = (tl) => {
    if (collectionDecor) {
      const collectionDecorItems = collectionDecor.querySelectorAll(".decors-collection__item");
      collectionDecorItems.forEach((item, i) => {
        tl.from(
          item,
          {
            scale: random(0.3, 0.7)
  ...
  if (window.matchMedia("(min-width: 1920px)").matches) {
    coolectionDecorInit(tlCollection);
  }
};
function gsapInit() {
  blockHeaderEffect();
